/**
 * @version		$Id: mediamanager.js 18748 2010-09-01 06:57:26Z eddieajau $
...
var MediaManager = this.MediaManager = {
	...
	initialize: function()
	{
		...
		this.tree = new MooTreeControl({ div: 'media-tree_tree', mode: 'folders', grid: true, theme: '../media/system/images/mootree.gif', onClick:
            ...
            },{ text: 'Repository', open: true, data: { url: 'index.php?option=com_jaextmanager&view=repolist&tmpl=component', target: 'folderframe'}});
	...
	onloadframe: function()
	{
		...
		if (folder) {
			...
			folderpath = folderpath.replace(/[\\\/]+/gi, '/');//cleanup path
	...
	setViewType: function(type)
	{
		...
		viewstyle = type;
		var folder = this.getFolder();
        this._setFrameUrl('index.php?option=com_jaextmanager&view=repolist&tmpl=component&folder='+folder+'&layout='+type);
	},
