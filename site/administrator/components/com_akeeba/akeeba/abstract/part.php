...
abstract class AEAbstractPart extends AEAbstractObject
{
	...
	final public function tick($nesting = 0)
	{
		...
		)
		{
			...
			$nesting++;
			if($this->nest_logging) AEUtilLogger::WriteLog(_AE_LOG_DEBUG, "*** Batching successive steps (nesting level $nesting)");
		...
		else
		{
			...
			if($this->nest_logging) {
				if($breakFlag) AEUtilLogger::WriteLog(_AE_LOG_DEBUG, "*** Engine steps batching: Break flag detected.");
