...
class AEArchiverZip extends AEAbstractArchiver {
	...
	protected function _addFile( $isVirtual, &$sourceNameOrData, $targetName )
	{
		...
		if(!$configuration->get('volatile.engine.archiver.processingfile',false)) {
			...
			if($isVirtual) AEUtilLogger::WriteLog(_AE_LOG_DEBUG, '  Virtual add:'.$storedName.' ('.$fileSize.') - '.$compressionMethod);
			...
			if( (function_exists('mb_strlen') ? mb_strlen($dtime,'8bit') : strlen($dtime)) < 8 ) $dtime = "00000000";
					...
					else
					{
						...
						else
						{
							...
							while( !feof($zdatafp) && ($timer->getTimeLeft() > 0) )
							{
								...
								if($chunk_size_primary <= 0) $chunk_size_primary = max(AKEEBA_CHUNK, $free_space);
...
class AECRC32CalcClass
{
	...
	private function crc32_file_php512($filename)
	{
		...
		if(is_null($mustInvert))
		{
			...
			if($mustInvert) AEUtilLogger::WriteLog(_AE_LOG_WARNING,'Your server has a buggy PHP version which produces inverted CRC32 values. Attempting a workaround. ZIP files may appear as corrupt.');
		}
